...
def _fallback_agent_response(prompt: str) -> str:
    ...
    # Extract the actual question from the generated prompt
    question = prompt
    if "══ السؤال ══" in prompt:
        question = prompt.split("══ السؤال ══")[-1].split("══ تعليمة التحليل ══")[0]
        
    prompt_lower = question.lower()
    
    if "تقرير" in prompt_lower or "report" in prompt_lower:
        return (
            f"{disclaimer}"
            f"### 📄 تقرير آلي (وضع الطوارئ)\n\n"
            f"تم إنشاء التقرير المبدئي بناءً على التحليل الآلي.\n"
            f"نظراً لأن الخادم المعرفي غير متصل حالياً، هذا التقرير يمثل تلخيصاً للبيانات المتاحة."
        )
    if any(w in prompt_lower for w in ["درون", "drone", "طائرة", "طائرات", "مسي"]):
        return (
            f"{disclaimer}"
            f"### 🚁 تقرير الطائرات المسيرة (Drone)\n\n"
            f"| المؤشر | القيمة المسجلة |\n"
            f"|---|---|\n"
            f"| النطاقات المستهدفة | `433 MHz`, `2.4 GHz`, `5.8 GHz` |\n"
            f"| الإجراء الفوري | إطلاق إنذار لحظي للمشغلين |\n"
            f"| التوجيه | الانتباه المستمر للنشاط القوي في هذه الترددات |\n"
        )
    if any(w in prompt_lower for w in ["تشويش", "jamming", "شوش", "هجوم"]):
        return (
            f"{disclaimer}"
            f"### 📡 تحليل هجمات التشويش (Jamming)\n\n"
            f"| المؤشر | القيمة المسجلة |\n"
            f"|---|---|\n"
            f"| الخصائص | طاقة عالية (SNR مرتفع) |\n"
            f"| التأثير | يمنع الاتصالات الطبيعية |\n"
