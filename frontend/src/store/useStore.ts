    ...
    (set, get) => ({
      ...
      fetchSignals: async (limit = 100, label?: string) => {
        ...
        try {
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to fetch signals', isLoading: false });
        }
      },
      ...
      fetchAlerts: async () => {
        ...
        try {
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to fetch alerts', isLoading: false });
        }
      },
      ...
      addAlert: (alert: Alert) => {
        set((state) => ({ alerts: [alert, ...state.alerts.slice(0, 99)] }));
      },
      ...
      refreshAll: async () => {
        ...
        await Promise.all([fetchSignals(), fetchAlerts(), fetchStatistics()]);
      },
    }),
    {
      name: 'sadar-storage',
